...
if($this->config->conf['use_calendar']){
	echo "<a title=\"".Kohana::lang('common.title-calendar-link')."\" href=\"#\" id=\"button\"><i class="material-icons">event</i></a>"; 
}
if($this->config->conf['use_fpdf'] == 1 && ( $position == "graph" || $position == "special") ){
	echo "<a title=\"".Kohana::lang('common.title-pdf-link')."\" href=\"".url::base(TRUE)."pdf".$qsa."\"><i class="material-icons">picture_as_pdf</i></a>\n";
}
if($this->config->conf['use_fpdf'] == 1 && $position == "basket"){
	echo "<a title=\"".Kohana::lang('common.title-pdf-link')."\" href=\"".url::base(TRUE)."pdf/basket/".$qsa."\"><i class="material-icons">picture_as_pdf</i></a>\n";
}
if($this->config->conf['use_fpdf'] == 1 && $position == "page"){
	echo "<a title=\"".Kohana::lang('common.title-pdf-link')."\" href=\"".url::base(TRUE)."pdf/page/".$this->page.$qsa."\"><i class="material-icons">picture_as_pdf</i></a>\n";
}
...
echo "<a title=\"".Kohana::lang('common.title-statistics-link')."\" href=\"".url::base(TRUE)."graph?host=.pnp-internal&srv=runtime\"><i class="material-icons">equalizer</i></a>\n";
...
echo "<a title=\"".Kohana::lang('common.title-docs-link')."\" href=\"".url::base(TRUE)."docs\"><i class="material-icons">help</i></a>\n";
?>
</div>
</div><p>
<!-- Icon Box End -->
